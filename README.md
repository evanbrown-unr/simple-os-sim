...
### Phase 4
The main change for this phase is that we are using a priority queue to order the PCBs and their respective operations. This differs from the previous phases because they used a FIFO queue which corresponds to a FCFS scheduling algorithm. In this phase we will be using priority scheduling, hence the replacement of a linked list with a priority queue implemented internally as a binary min heap. The meta data file layout must be changed a little bit as well. Each metadata operation requires an extra field representing a priority level for each PCB and operation within each PCB. The memory requirements are also subject to being modified. We will likely implement some method for handling physical vs virtual memory management. This is TBD.